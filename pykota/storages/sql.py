...
class SQLStorage :
    ...
    def getMatchingStuff(self, pattern, tablename, entrytype, keyname) :
        ...
        if self.hasWildCards(pattern) :
            ...
            if result :
                for record in result :
                    name = self.databaseToUserCharset(record[keyname])
            ...
            while patterns :
                ...
                if nbpatterns == 1 :
                    wherestmt = "%s=%s" % (keyname, self.doQuote(self.userCharsetToDatabase(subset[0])))
                else :
                    wherestmt = "%s IN (%s)" % (keyname,
                                                ",".join([self.doQuote(self.userCharsetToDatabase(p)) for p in subset]))
