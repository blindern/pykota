    ...
    def getMatchingUsers(self, userpattern) :
        ...
        if self.hasWildCards(userpattern) :
            ...
            while patterns :
                ...
                if nbpatterns == 1 :
                    wherestmt = "username=%s" % self.doQuote(self.userCharsetToDatabase(subset[0]))
                else :
                    wherestmt = "username IN (%s)" % ",".join([self.doQuote(self.userCharsetToDatabase(p)) for p in subset])
